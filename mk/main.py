...
class MKPartitioner(GreedPartitioner):
    ...
    def mk_nparts(self, G: nx.Graph, nparts: int, cr_max: float, max_ufactor: float | None = 1e7, weighted: bool = True, check_cache: bool = True, steps_back: int = 5) -> tuple[nx.Graph, list[int]] | tuple[None, None]:
        ...
        partition_ans = super().do_metis(G, nparts, cr_max, steps_back=steps_back)
    ...
    def mk_part(self, G: nx.Graph, cr_max: float, check_cache: bool = True, steps_back: int = 5) -> tuple[int, list[int]]:
        if check_cache:
    ...
    def get_num_mk(self, G: nx.Graph, cr_max: float, steps_back: int = 5, check_cache: bool = True) -> int:
        (n, _) = self.mk_part(G, cr_max, steps_back=steps_back, check_cache=check_cache)
    ...
    def MK_greed_greed(self, G: nx.Graph, PG: nx.Graph, cr_max: float, steps_back: int = 6, check_cache: bool = True) -> list[int] | None:
        max_mk = self.get_num_mk(G, cr_max, steps_back=steps_back, check_cache=check_cache)
        ...
        for nparts in range(1, max_mk + 1):
            (G_grouped, mk_data) = self.mk_nparts(G, nparts, cr_max, check_cache=check_cache, steps_back=steps_back)
    ...
    def do_MK_greed_greed(
        ...
        partition = self.MK_greed_greed(weighted_graph, physical_graph, cr_max, steps_back=steps_back, check_cache=check_cache)
    ...
    def MK_greed_greed_with_geq_cr(self, G: nx.Graph, PG: nx.Graph, cr_max: float, check_cache: bool = True, steps_back: int = 6) -> list[int]:
